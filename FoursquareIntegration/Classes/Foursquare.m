...
#define XAUTH_ACCESS_TOKEN_URL	@"https://foursquare.com/oauth/access_token"	// URL for request access token
								  ...
								  password:(NSString *)password
{
    ...
    URLLoader * loader = [[URLLoader alloc] initWithRequest:request];
    [loader addTarget:self onLoad:@selector(requestXAuthAccessTokenForUsernamePasswordLoad:)];
    [loader load];
...
- (void)requestXAuthAccessTokenForUsernamePasswordLoad:(URLLoader*)loader
{
    NSData * data = [loader data];
    NSHTTPURLResponse * response = (NSHTTPURLResponse*)loader.response;
    if(data != nil && response != NULL && [response statusCode] >= 200 && [response statusCode] < 300)
	{
		NSLog(@"Foursquare: received oAuth Token");
	...
	{
		NSString *text = [[NSString alloc] initWithData:data encoding:NSUTF8StringEncoding];
		NSLog(@"Foursquare: could not receive a valid oAuth Token %@ %@", response, text);
	}
    if ([delegate respondsToSelector:@selector(foursquareLoginWithUsernamePasswordLoaded:)]){
        [delegate performSelector:@selector(foursquareLoginWithUsernamePasswordLoaded:) withObject:[NSNumber numberWithBool:[self isActive]]];
    }
    [loader release];
}
